...
export class UserService {
	...
	async authenticate(request: Request): Promise<User | undefined> {
		...
		const userResult = await this.db
			.select()
			.from(users)
			.where(or(eq(lower(users.email), email.toLowerCase()), eq(lower(users.pseudo), email.toLowerCase())))
			.get();
		...
		await this.db.update(users).set({ token: newToken, tokenValidity: tokenValidity.getTime() }).where(eq(users.id, userResult.id));
	...
	async validateToken(requestToken: string | undefined, admin: boolean = false): Promise<User | Response> {
		...
		const tokenValidity = new Date();
		tokenValidity.setDate(tokenValidity.getDate() + 1);
		await this.db.update(users).set({ tokenValidity: tokenValidity.getTime() }).where(eq(users.id, userResult.id));
	...
	public async changeUserState(request: Request, pseudo: string) {
		const body: { ready: boolean | undefined; canPlayTarot: boolean | undefined; canPlayTwoTables: boolean | undefined } =
			await request.json();
		...
		if (user) {
			await this.db
				.update(users)
				.set({
					ready: body.ready ?? user.ready,
					canPlayTarot: body.canPlayTarot ?? user.canPlayTarot,
					canPlayTwoTables: body.canPlayTwoTables ?? user.canPlayTwoTables,
				})
	...
	public async quit(pseudo: string): Promise<number | undefined> {
		...
		if (user) {
			const tokenValidity = new Date();
			tokenValidity.setDate(tokenValidity.getDate() - 1);
			await this.db.update(users).set({ tokenValidity: tokenValidity.getTime() }).where(eq(users.id, user.id));
			return user.id;
		}
	}
	...
	async genMissingTokens() {
		...
		for await (const user of missingTokenAdminUsers) {
			...
			await this.db.update(users).set({ token: newToken, tokenValidity: tokenValidity.getTime() }).where(eq(users.id, user.id));
		}
	...
	async adminGenerateToken(userId: number) {
		...
		const tokenValidity = new Date();
		tokenValidity.setDate(tokenValidity.getDate() + 1);
		await this.db.update(users).set({ token: newToken, tokenValidity: tokenValidity.getTime() }).where(eq(users.id, userId));
		const base64Token = Buffer.from(newToken).toString('base64');
		return new Response(JSON.stringify({ token: base64Token }), { status: 200 });
	}
}
