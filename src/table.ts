...
const generateTable = function (
	players: User[],
	playerOnTwoTable: User | undefined,
	gameMode: GameMode,
	usedTableNames: string[],
	teamsNeeded: number,
	panamaTable: FullTable,
): FullTable {
	let teams: Team[] = [];

	for (let index = 0; index < teamsNeeded; index++) {
		let users: User[] =
			teamsNeeded == players.length
				? [players[index]]
				: [
	...
	panamaTable.teams[0].users = panamaTable.teams[0].users.filter(
		(user) => !players.find((userSelected) => userSelected.pseudo === user.pseudo),
	);
	return fullTable;
};
